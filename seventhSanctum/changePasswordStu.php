...
<br><br>
    ...
    <!-- <?php if(isset($_POST['click'])){echo"<button class='sub'><a href='stHome.php'>Home</a></button>";} ?> -->
...
if($npass!=$conpass){
    echo"<h5>Both Passwords don't match.</h5>";
        ...
        $sql2="SELECT email FROM students WHERE email='$email' and password='$opass'";
        $res=$conn->query($sql2);
        if($res->num_rows==0){
            echo"<h5>You have entered Incorrect Old Password</h5>";
        }else{
            $sql3="UPDATE students SET password ='$npass' WHERE email='$email'";
            $res=$conn->query($sql3);
            $upd=1;
    ...
    if($upd==1){
        echo"<button class='sub'><a href='stHome.php'>Home</a></button>";
    }
